...
//
// (DRY RUN)
// node scripts/mint.js ./tmp/members.json
// (RUN)
// node scripts/mint.js ./tmp/members.json --dry-run=false
//
...
async function mintAndTrasfer(batchMembers, dryrun = true, logPrefix = null) {
  ...
  try {
    ...
    const addresses = batchMembers.map((member) => member.address)
